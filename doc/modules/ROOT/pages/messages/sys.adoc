...
|===
|Name |Required? |Type |Description
|`timestamp` |yes |xref:types.adoc#_timestamp[timestamp] |The current UNIX timestamp on the server in milliseconds at the time when the message was received if no adjustment was performed, or the timestamp _after_ the adjustment if an adjustment was performed.
|===
