...
|===
|Name |Required? |Type |Description
|`items` |yes |array of xref:types.adoc#_logmessage[`LogMessage`] |The array of messages to send (must contain at least one item)
|===
...
service names yet, but the reference implementation of the server uses service
...
|===
|Name |Required? |Type |Description
|`timestamp` |yes |integer |The current UNIX timestamp on the server in milliseconds at the time when the message was received if no adjustment was performed, or the timestamp _after_ the adjustment if an adjustment was performed.
|===
