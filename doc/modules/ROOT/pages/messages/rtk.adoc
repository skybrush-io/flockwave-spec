...
xref:types.adoc#rtk-antenna-info[`RTKAntennaInfo`] complex type.
...
xref:types.adoc#rtk-message-observations[`RTKMessageObservations`] complex type.
...
xref:types.adoc#rtk-survey-status[`RTKSurveyStatus`] complex type.
...
[source,json]
----
include::example$request_RTK-STATUS.json[]
----
...
[source,json]
----
include::example$response_RTK-STATUS.json[]
----
...
RTK base station to either to start an RTK auto-survey to measure its own position, 
or to set its position explicitely.
