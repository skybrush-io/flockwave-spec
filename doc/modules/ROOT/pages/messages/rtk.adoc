...
enhancing GNSS positioning accuracy.
...
== `RTK-SOURCE` — Query or select an RTK source managed by the server

A client sends this request to the server to either query the current RTK
preset or to select an RTK preset that will be used as the active RTK base
station that sends RTK corrections to the UAVs through the server.

The response will be another `RTK-SOURCE` packet when querying the current
RTK source, or an `ACK-ACK` when setting the RTK source.
...
|===
|Name |Required? |Type |Description
|`id` |no |string |The ID of the RTK preset to use; omitted when querying the preset, `null` when clearing the currently selected preset
|===
...
|===
|Name |Required? |Type |Description
|`id` |yes |string |The ID of the RTK preset selected as RTK base; `null` if no preset is selected
|===
