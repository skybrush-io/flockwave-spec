...
This type represents a coordinate given by a GPS in the
"Earth-centered Earth-fixed" format using the geocentric WGS84
...
All three coordinates (`X`, `Y`, and `Z`) are stored in meters.
...
|===
|Index |Required? |Type |Description
|0 |yes |float |The `X` coordinate in meters
|1 |yes |float |The `Y` coordinate in meters
|2 |yes |float |The `Z` coordinate in meters
|===
...
The `RTKAntennaInfo` object holds information about the current RTK antenna.
...
----
{
    ...
    "serialNumber": "TODO",
    "position": [519976597, -7406863, 93765],
    "positionECEF": [4120339, 1418747, 4641838]
}
----


[#rtk-message-observations]
== `RTKMessageObservations`

The `RTKMessageObservations` object contains statistical information about 
...
|position |no |<<_gpscoordinate,GPSCoordinate>> |The RTK base station antenna position in GPS coordinates
|gnssTypes |no |list of strings | The GNSS types to use (choose from "gps", "glonass", 
"galileo", "sbas", "qzss", "beidou", "irnss")
...
The `RTKSurveyStatus` object contains status of the current RTK base station
self-survey procedure.
...
|===
|Index |Required? |Type |Description
|0 |yes |float |Accuracy of the antenna position during the survey in meters
|1 |yes |integer |RTK survey status flags:
