...
inaccuracies]). Therefore, the typical range of angles is [0, 3600) or
...
|stationId |no |integer |The station ID of the antenna, or null if not known
|descriptor |no |string |The antenna descriptor, or null if not known
|serialNumber |no |string |The serial number of the antenna, or null if not known
|position |no |<<_gpscoordinate,GPSCoordinate>> |The position of the
RTK antenna, in GPS coordinates, or null if not known
|positionECEF |no |<<_ecefcoordinate,ECEFCoordinate>> |The position of
...
An `AuthorizationScope` object specifies the scope of the authorization to
start a drone show. This type should be interpreted in conjunction with the
<<_startconditions,StartConditions>> object.

The scope may be one of the following values:

`none`:: The show is not authorized to start.

`live`:: The show is authorized to start in a live setting, with all safety
features enabled.

`rehearsal`:: The show is authorized to start in a rehearsal setting, with
some or all of the safety features disabled for testing purposes. This should
be used only if the flight takes place in a controlled environment, far from
the audience.

`lights`:: The show is authorized to start with lights only; the UAVs
participating in the show are not allowed to start their motors.

Earlier versions of this specification did not contain the `AuthorizationScope`
enum and <<_startconditions,StartConditions>> did not contain a field of this
type either. For sake of backwards compatibility, the `authorizationScope` field
in <<_startconditions,StartConditions>> is therefore optional. When it is not
provided, its value is assumed to be `none` if the authorization is not granted
and `live` if the authorization is granted.
...
`cartesian`:: A channel that provides a Cartesian coordinate.
...
`gps`:: A channel that provides a GPS coordinate.
...
|===
|Name |Required? |Type |Description
|`id` |yes |<<_receiptid,`ReceiptId`>> |Identifier of the command execution

|`created_at` |yes |<<_timestamps,timestamp>> |Timestamp when the command execution object was created

|`client_notified` |no |<<_timestamps,timestamp>> |Timestamp when the client was notified about the result

|`error` |no |string |Error message if the command execution failed

|`result` |no |any |Result of the command execution

|`sent` |no |<<_timestamps,timestamp>> |Timestamp when the command was sent to the UAV

|`finished` |no |<<_timestamps,timestamp>> |Timestamp when the command execution finished

|`cancelled` |no |<<_timestamps,timestamp>> |Timestamp when the command execution was cancelled

|`progress` |no |xref:#_progress[`Progress`] |Progress of the command execution
|===
...
{
    "id": "abc123",
    "created_at": 1459670842000,
    "sent": 1459670842000,
    "finished": 1459670842811,
    "progress": {
        "percentage": 100
    }
}
...
`rangefinder`:: The device is a rangefinder (e.g., a laser or sonic range finder).
...
`sprayer`:: The device is a sprayer (e.g., a crop sprayer).
...
|===
|Name |Required? |Type |Description
|start |yes | <<_startconditions,StartConditions>> |
...
`rth`:: The UAV is returning to its home position autonomously, optionally landing
...
This type represents an abstract component (software or hardware) of an object
that can be targeted with a firmware update operation. Firmware update targets
can be registered and unregistered in the server and each object can report
the list of firmware targets that it supports. An object may have multiple
firmware update targets or may have none; for instance, a UAV consisting of an
autopilot and a telemetry module can provide two firmware update targets if the
firmware can be updated on both components remotely.

The firmware of a component of an object may be updated by sending a firmware
update request to the server, indicating the object, the firmware update target
and the firmware itself; the server will then take care of orchestrating the
replacement of the firmware on the object.
...
|===
|Name |Required? |Type |Description
|`id` |yes |string |The unique identifier of the firmware update target
|`name` |yes |string |A human-readable name of the firmware update target
|===
...
Configuration of predefined LED light effects in shows. This type is used
by the `SHOW-ADAPT` command's transformation parameters.

*Fields*

The actual set of fields depends on the `type` discriminator field:

`default`:: The default lighting method with informative but not disturbing lights.
`off`:: Lights turned off.
`original`:: Keep the existing light configuration unchanged as much as possible.
`solid`:: A solid, single-color light effect.
`sparks`:: Colored sparks with random exponential distribution.

--
*Fields of the `default` type*
[width="100%",cols="15%,10%,25%,50%",options="header",]
|===
|Name |Required? |Type |Description
|`type` |yes |string |Must be `"default"`
|`brightness` |no |float |Brightness of the light in the [0, 1] interval. Default brightness is 0.05.
|===
--
*Fields of the `off` type*
[width="100%",cols="15%,10%,25%,50%",options="header",]
|===
|Name |Required? |Type |Description
|`type` |yes |string |Must be `"off"`
|===
--
*Fields of the `original` type*
[width="100%",cols="15%,10%,25%,50%",options="header",]
|===
|Name |Required? |Type |Description
|`type` |yes |string |Must be `"original"`
|===
--
*Fields of the `solid` type*
[width="100%",cols="15%,10%,25%,50%",options="header",]
|===
|Name |Required? |Type |Description
|`type` |yes |string |Must be `"solid"`
|`color` |no |string |The solid color to use, according to the CSS3 Color Module Level 3 specification (named strings, hex strings, and RGB tuples are accepted). Default color is white.
|===
--
*Fields of the `sparks` type*
[width="100%",cols="15%,10%,25%,50%",options="header",]
|===
|Name |Required? |Type |Description
|`type` |yes |string |Must be `"sparks"`
|`color` |no |string |The color of the sparks, according to the CSS3 Color Module Level 3 specification (named strings, hex strings, and RGB tuples are accepted). Default color is white.
|`off_duration` |no |float |Duration of the average off period between sparks, in seconds. Default is 3 seconds.
|===
--
...
|===
|Index |Required? |Type |Description
|0 |yes |integer |North component, in nanoteslas
|1 |yes |integer |East component, in nanoteslas
|2 |yes |integer |Down component, in nanoteslas
|===
...
|1 |yes |float |The average bitrate of the message stream in the last 10 seconds, in bits per second
|===
...
|===
|Index |Required? |Type |Description
|0 |yes |integer |The X component of the position vector, in mm
|1 |yes |integer |The Y component of the position vector, in mm
|2 |yes |integer |The Z component of the position vector, in mm
|===
...
An `RSSI` object is an array of integers, each holding the received signal
...
Received signal strength is expressed on an abstract, unitless scale from 0
to 100. Percentages may be used directly if the UAV can provide an RSSI reading
that is already mapped into a percent scale. -1 is also a valid value,
interpreted as "no RSSI reading available".
...
* *Built-in* presets are supplied by the configuration of the server and may not
  be modified or removed by the user.

* *Dynamic* presets are created by the server automatically based on the hardware
  configuration. For instance, USB serial ports may have one corresponding
  preset that assumes that an RTK base station is attached to the port. These
  presets may not be modified or removed by the user either.

* *User* presets are created by the user manually and may be modified, added or
  removed on-demand.
...
|type |no |string |Type of the RTK preset ("builtin", "dynamic", "user")
...
|messageSet |no |string | The message set to use / used ('msm4' or 'msm7')
...
== `RthTransformationParameters`

Parameters for adapting the ending (RTH) segment of the drone show.
Used by the `SHOW-ADAPT` command.
...
|===
|Name |Required? |Type |Description
|`method` |no |string |The RTH method to use. Accepted values: `"plain"` (brings the drones to the possibly layered floating grid of the end of their takeoff positions with a collectively optimal transition, _not_ preserving individual home positions. A final integrated land phase follows), `"smart"` (brings the drones to their individual home positions, while avoiding drones being closer to each other than the minimum distance threshold thoughout the entire RTH and its final integrated land phase). Default: `"smart"`
|`lights` |no |xref:#_lighteffectconfiguration[`LightEffectConfiguration`] or float |Light effect configuration for the drones during RTH, or a brightness value in the [0, 1] interval to use with the default light configuration
|`min_distance` |no |float |Minimum distance to keep between drones during RTH, in meters. If not specified, the configured validation setting will be used.
|`velocity_xy` |no |float |The horizontal RTH speed to use, in m/s. Default value is 5 m/s.
|`velocity_z` |no |float |The vertical RTH and land speed to use, in m/s. Default value is 2 m/s.
|`altitude` |no |float |The altitude of the smart RTH from where vertical land is called, in meters. If not specified, the minimum navigation altitude will be used as a default.
|`replace` |no |boolean |Whether to replace the existing RTH and land segment if the show segment is defined. Default: false
|===
...
== `ShiftTransformationParameters`

Parameters for shifting the altitude of the show segment. Used by the
`SHOW-ADAPT` command.
...
|===
|Name |Required? |Type |Description
|`z` |no |float |Vertical shift to apply to the show segment, in meters
|===
...
== `TakeoffTransformationParameters`

Parameters for adapting the beginning (takeoff) segment of the drone
show. Used by the `SHOW-ADAPT` command.
...
|===
|Name |Required? |Type |Description
|`positions` |no |array of [x, y, z] or null |Explicit takeoff positions to use, as [x, y, z] coordinates in meters relative to the show origin. If empty, original takeoff positions of drones will be used instead.
|`duration` |no |float |Optional overall duration of the transformation to set, in seconds
|`method` |no |string |The takeoff method to use. Accepted values: `"layered"` (takeoff to a floating grid from original takeoff positions or explicit positions provided, then morph to the first formation of the show), `"organic"` (takeoff with a reversed smart RTH procedure, that provides an organic look and also preserves mapping between takeoff and formation positions). If not specified, it will be inferred from other parameters.
|`lights` |no |xref:#_lighteffectconfiguration[`LightEffectConfiguration`] or float |Light effect configuration for the drones during takeoff, or a brightness value in the [0, 1] interval to use with the default light configuration
|`min_distance` |no |float |Minimum distance to keep between drones during multi-layer takeoff and transition to the first formation, in meters. If not specified, the configured validation setting will be used.
|`velocity_xy` |no |float |The horizontal speed to use towards the first formation, in m/s. Default is 5 m/s.
|`velocity_z` |no |float |The vertical speed to use during takeoff and towards the first formation, in m/s. Default value is 1.5 m/s.
|`altitude` |no |float |The takeoff height of the last (lowest) layer before any horizontal motion is planned, in meters. Default value is 10 m.
|`replace` |no |boolean |Whether to replace the existing takeoff segment if the show segment is defined. Default: false
|===
...
----
{
    "positions": [[5, 5, 5], [6, 5, 5], [7, 5, 5], [8, 5, 5]],
    "method": "layered",
    "velocity_xy": 5,
    "velocity_z": 1.5,
    "altitude": 10,
...
|===
|Name |Required? |Type |Description
|authorized|yes | boolean | Whether the server has received authorization from a human operator to start the show
|authorizationScope|no | <<_authorizationscope,AuthorizationScope>> | Scope of the authorization. Not mandatory for sake of backward compatibility with older versions. When the scope is missing, it is assumed to be `"live"` if the authorization is granted and `"none"` if it is not.
|clock|yes | string or null| Identifier of the clock that the start time refers to; null means that the start time is a UNIX timestamp
|time|yes | <<_timestamps,timestamp>> | Timestamp when the drone show should start
|method|yes | <<_startmethod,StartMethod>> | Method of starting the show. Can be either `rc` or `auto`.
|uavIds|yes | list of strings | Array containing the IDs of the UAVs that should be started automatically.
|===
...
A `TimeSyncSnapshot` object describes the current estimate of whether the
server clock is synchronized to the world clock.
...
|===
|Name |Required? |Type |Description
|`state` |yes |string |Current time synchronization state. One of `unknown`
(unknown state), `sync` (the server clock is synchronized to the world clock),
`unsync` (the server clock has a significant offset compared to the world
clock) or `error` (all time sources reported an error, so the synchronization
state cannot be determined).

|`offset` |yes |float or `null` |Offset between the server clock and the world
clock, in seconds; `null` if not known.

|`source` |yes |string or `null` |Identifier of the time source that was used to
find the offset; `null` if not known.

|`jitter` |yes |float or `null` |Jitter of the time source that was used to find
the offset; `null` if not known.
|===
...
[#time-axis-schedule-segment]
== `TimeAxisScheduleSegment`
...
|===
|Name |Required? |Type |Description
|`type` |yes |string |Type of the time axis segment. One of `preparation`, `rth`, `show`, `slowdown` or `speedup`.

|`startMs` |yes |integer |Start of the segment in world time seconds, relative to the Unix epoch.

|`endMs` |yes |integer, `"auto"` or `"inf"` |End of the segment in world time seconds, relative to the Unix epoch. `"auto"` means the end time is not yet calculated and will be determined by the receiver. `"inf"` means the segment runs indefinitely.

|`params` |no |object |Additional parameters for the segment.
|===
...
[#collective-rth-plan-stat-entry]
== `CollectiveRthPlanStatEntry`

Statistics about a single collective RTH (return-to-home) plan in a show.
...
|===
|Name |Required? |Type |Description
|`time` |yes |float |Start time of the collective RTH plan relative to show start, in seconds

|`duration` |yes |float |Duration of the collective RTH operation without landing, in seconds

|`showDuration` |yes |float |Total show duration including collective RTH and landing, in seconds
|===
...
|rssi |no |<<_rssi,RSSI>> |The received signal strength from the UAV,
for each of the communication channels it is accessible on.
...
|===
|Name |Required? |Type |Description
|stationId |no |string |Identifier of the weather station that provided the data
